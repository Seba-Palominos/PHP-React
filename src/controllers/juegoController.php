...
class juegoController{
    ...
    public function pertenece(Request $request, Response $response){
        ...
        $datosPartida = !Juego::guardarPartida($idUsuario,$fecha->format('Y-m-d H:i:s'),$datos['idMazo'],'en_curso');
        if($datosPartida ){
        ...
        return Respuesta::respuesta($response,['id_partida'=> $datosPartida['id']],200);
        }catch(Exception $e){
